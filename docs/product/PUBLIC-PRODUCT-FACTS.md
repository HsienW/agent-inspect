# Public product facts

Single source of public product truth for README, website, npm descriptions, AI assets, and validators.

**Do not** put internal adoption ledgers, v7 gates, or release-train chunk IDs here.

## Identity

| Field | Value |
|-------|-------|
| Name | agent-inspect |
| Category | Local-first evidence for TypeScript AI agents |
| Headline | See what your agent did. Catch the wrong path in CI. Keep the evidence local. |
| Outcome | Capture once. Debug, prevent, and share from the same local trace. |
| Version | 6.22.0 |
| Schema | 1.0 |
| Node | >=20 |
| License | MIT |
| Public packages | 18 (Changesets fixed group) |
| Status line | Current release: 6.22.0 · schema 1.0 · Node.js 20+ · MIT · actively maintained |
| Maintenance | The 6.22 line is actively maintained for correctness, compatibility, documentation, security, and framework evolution. |

## Supporting sentence

AgentInspect turns TypeScript agent runs into readable execution trees, deterministic trajectory checks, and portable Evidence v2—without requiring an account, collector, or default upload.

## Three jobs (one local trace)

1. **Debug** — Read nested steps, tools, model metadata, and the first causal failure from local JSONL.
2. **Prevent** — Deterministic check presets, TraceContract, suites, and CI gates — no LLM judge required.
3. **Share** — Share-checked Evidence v2 with integrity verification. Not a compliance certification.

Optional Preview: read-only MCP over the same local facts.

## Proof language (public-safe)

- Zero open pilot findings at the 6.16.0 moderate + deep-swarm gates.
- Hardening timeline 6.7.3 → 6.16.0; Evidence UX in 6.17.1; fixture-backed demos.

Do not name companies, customers, or retention counts without explicit permission.

## Trust / boundaries

- No account · no collector · no default upload · metadata-only by default
- Customer-owned artifacts
- No compliance-certification claim
- No production APM replacement claim
- Studio and MCP remain explicit opt-in / local read-only

## Banned public status phrases

Do not use on active public surfaces (README, website marketing, package READMEs, docs home, FAQ, llms.txt):

- technical launch candidate / launch candidate only
- adoption freeze / feature freeze / waiting for adoption
- external pilot evidence pending (as product status)
- blocked on external evidence / hold / calendar wait
- v7 not scheduled (marketing FAQ/homepage)
- MCP-ready
- Four pillars
- share-safe (use share-checked)
- The 6.16 line is actively maintained
- pending re-record

Machine-readable twin: [PUBLIC-PRODUCT-FACTS.json](./PUBLIC-PRODUCT-FACTS.json).
