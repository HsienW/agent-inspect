...
2. **Read versus write.** Default `sideEffectClass` is `"read"`. Read recovery may pass with noSideEffect / digest-or-structured argument evidence. Write tools treat `running` / `unknown` / timeout-like completion as **unevaluable → fail**. A client `idempotencyKey`, `noSideEffect: true`, or `sideEffect: false` records intent or producer claims only — they do **not** clear write-completion unevaluability for `retry.operations`. Flagship recipe focuses on read-only `retrieve_policy`.
...
4. **Non-goals.** AgentInspect is **not** a retry engine: it does not schedule, remediates, or execute retries. Write-retry is **not** safe by default. A client `idempotencyKey` is not proof of server deduplication, authoritative result, or exactly-once mutation. Weaker `retry.requireIdempotencyEvidenceForRetry` intent checks remain separate. No Temporal/workflow DSL, no schema 1.1, no network.
