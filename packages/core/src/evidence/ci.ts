 ...
 * Order (6.29.3): resolve → safety → bind → HTML → manifest last.
...
export function buildEvidenceCiPackage(input: EvidenceCiPackageInput): EvidenceCiPackageFiles {
  ...
  const evidenceHtml = buildEvidenceHtmlShell({
    ...
    ...(contractBinding !== undefined
      ? {
          contractBinding: {
            status: contractBinding.status,
            ...(contractBinding.sha256 !== undefined
              ? { sha256: contractBinding.sha256 }
              : {}),
            ...(contractBinding.note !== undefined ? { note: contractBinding.note } : {}),
            ruleCount: contractBinding.ruleIds.length,
          },
        }
      : {}),
  });
