...
/**
 * Lazily load better-sqlite3 when index build/query starts.
 * Importing `@agent-inspect/index-sqlite` must not require the native driver.
 */
  ...
  try {
    ...
    throw new Error(
      `better-sqlite3 native driver is unavailable (${detail}). ` +
        `Install a prebuild for your Node/OS (see better-sqlite3 docs) or fall back to JSONL directory scans without the SQLite index. ` +
